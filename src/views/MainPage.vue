...
import { DATA_FILE_NAMES, Session, Event, SessionState, detectedKeys, Detected } from "@/types";
...
setInterval(() => {
  ...
  Promise.all(fetchPromises)
    .then(results => {
      ...
      if (!currentlyDetected.isPresent && [SessionState.BREAK, SessionState.WORK].includes( global.value.sessions[0].state)) {
        // person is gone, pause the session
        global.value.sessions[0].state = SessionState.PAUSED;
      } else if (!global.value.lastDetected.isPresent && currentlyDetected.isPresent) {
        // person is back, resume the session
        global.value.sessions[0].state = global.value.stateBeforePause;
      } else if (currentlyDetected.isDistracted || currentlyDetected.isLookingAway) {
